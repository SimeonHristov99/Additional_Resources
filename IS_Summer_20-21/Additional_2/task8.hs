main :: IO()
main = do
    print $ highestCapital [(Country "Bulgaria" "Sofia" [(City "Varna" 0 16), (City "Plovdiv" 120 14), (City "Sofia" 420 13)]), (Country "Germany" "Berlin" [(City "Munchen" 200 15), (City "Berlin" 150 12), (City "Ulm" 210 15)]), (Country "France" "Paris" [(City "Paris" 180 15), (City "Nice" 0 14), (City "Lyon" 500 13)])] == "Bulgaria"

type Name = String
type Capital = Name
type AvgYearlyTemperature = Double
type Elevation = Int
data City = City Name Elevation AvgYearlyTemperature
 deriving (Show)
data Country = Country Name Capital [City]
 deriving (Show)

highestCapital :: [Country] -> Name
highestCapital countries = fst (foldr1 chooseHigher (map mapToHeight countries))
 where
  chooseHigher c1@(_, height1) c2@(_, height2) = if height1 <= height2 then c2 else c1
  mapToHeight (Country name capital cities) = (name, (head [height | (City name height _) <- cities, name == capital]))